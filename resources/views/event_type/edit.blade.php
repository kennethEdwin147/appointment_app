            ...
            <form method="POST" action="{{ route('event_type.update', $eventType) }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                           name="name" id="name" value="{{ old('name', $eventType->name) }}" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror"
                              name="description" id="description" rows="3">{{ old('description', $eventType->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" class="form-control @error('default_duration') is-invalid @enderror"
                           name="default_duration" id="default_duration" value="{{ old('default_duration', $eventType->default_duration) }}" min="1" max="1440" required>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" class="form-control @error('default_price') is-invalid @enderror"
                           name="default_price" id="default_price" value="{{ old('default_price', $eventType->default_price) }}" min="0">
                    @error('default_price')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" class="form-control @error('default_max_participants') is-invalid @enderror"
                           name="default_max_participants" id="default_max_participants" value="{{ old('default_max_participants', $eventType->default_max_participants) }}" min="1">
                    @error('default_max_participants')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

                @include('partials.meeting-platform-select', ['eventType' => $eventType])
