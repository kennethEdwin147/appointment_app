...
<body>
    ...
    <form method="POST" action="{{ route('availabilities.store') }}">
        ...
        <div>
            ...
            <select id="event_type_id" name="event_type_id" required>
                ...
                @foreach ($eventTypes as $eventType)
                    <option value="{{ $eventType->id }}">{{ $eventType->name }} (Durée: {{ $eventType->duration }} minutes)</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <div>
                <input type="radio" id="calendar_managed" name="availability_type" value="calendar_managed" {{ old('availability_type', 'calendar_managed') === 'calendar_managed' ? 'checked' : '' }}>
                <label for="calendar_managed">Ponctuelle</label>
                <input type="radio" id="repeating" name="availability_type" value="repeating" {{ old('availability_type') === 'repeating' ? 'checked' : '' }}>
                <label for="repeating">Récurrente</label>
            </div>
        </div>

        <div id="calendar_managed_fields" style="{{ old('availability_type', 'calendar_managed') === 'repeating' ? 'display: none;' : '' }}">
            <div>
        ...
        <div id="repeating_fields" style="{{ old('availability_type') !== 'repeating' ? 'display: none;' : '' }}">
            <div>
                <label>Répéter le :</label>
                <div>
                    <input type="checkbox" name="repeat_on[]" value="1" {{ in_array(1, old('repeat_on', [])) ? 'checked' : '' }}> Lundi
                    <input type="checkbox" name="repeat_on[]" value="2" {{ in_array(2, old('repeat_on', [])) ? 'checked' : '' }}> Mardi
                    <input type="checkbox" name="repeat_on[]" value="3" {{ in_array(3, old('repeat_on', [])) ? 'checked' : '' }}> Mercredi
                    <input type="checkbox" name="repeat_on[]" value="4" {{ in_array(4, old('repeat_on', [])) ? 'checked' : '' }}> Jeudi
                    <input type="checkbox" name="repeat_on[]" value="5" {{ in_array(5, old('repeat_on', [])) ? 'checked' : '' }}> Vendredi
                    <input type="checkbox" name="repeat_on[]" value="6" {{ in_array(6, old('repeat_on', [])) ? 'checked' : '' }}> Samedi
                    <input type="checkbox" name="repeat_on[]" value="0" {{ in_array(0, old('repeat_on', [])) ? 'checked' : '' }}> Dimanche
                </div>
            </div>
            <div>
                <label for="start_time_daily">Heure de début quotidienne</label>
                <input type="time" id="start_time_daily" name="start_time_daily" value="{{ old('start_time_daily') }}">
            </div>
            <div>
                <label for="end_time_daily">Heure de fin quotidienne</label>
                <input type="time" id="end_time_daily" name="end_time_daily" value="{{ old('end_time_daily') }}">
            </div>
            <div>
                <label for="repeat_start_date">Date de début de la récurrence</label>
                <input type="date" id="repeat_start_date" name="repeat_start_date" value="{{ old('repeat_start_date', now()->toDateString()) }}">
            </div>
            <div>
                <label for="repeat_end_date">Date de fin de la récurrence (facultatif)</label>
                <input type="date" id="repeat_end_date" name="repeat_end_date" value="{{ old('repeat_end_date') }}">
            </div>
        </div>
